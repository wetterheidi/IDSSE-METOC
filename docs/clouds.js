/**
 * VENDORED KOPIE von `meteokit/src/clouds.js` (Stand 2026-08-19), unverändert.
 *
 * IDSSE-METOC läuft ohne Build-Schritt (reine ES-Module, per nginx aus `docs/`
 * ausgeliefert) und kann `meteokit` daher nicht wie `droneforecast`/`trajectories`
 * live über Vite/`file:`-Dependency einbinden. Diese Datei ist deshalb eine
 * manuelle Kopie, keine Re-Exportierung. Bei Kalibrierungs-Updates in
 * `meteokit/src/clouds.js` (siehe dortige Hinweise zu `scripts/calibrate-clouds.mjs`)
 * muss diese Kopie manuell nachgezogen werden.
 *
 * Genutzt werden hier nur `cloudCeiling()` und `lowestCloudBase()` (für die
 * Metriken `cloudCeiling`/`cloudBase` in weather.js, nur für Modelle mit
 * nativen Modell-Level-Wolkendaten von Michaels Instanz — siehe
 * MICHAEL_LEVEL_CLOUD_MODELS in config.js). Der Rest der Datei (Fog-Diagnose,
 * Stockwerk-Bedeckung, LCL-Fallback) ist unverändert mitkopiert, um die Datei
 * 1:1 diffbar zum Original zu halten.
 */
